...
client.once('ready', async () => {
    ...
    try {
        ...
        await client.application.commands.set(REGISTERED_COMMANDS);
        for (const guild of client.guilds.cache.values()) {
            await guild.commands.set(REGISTERED_COMMANDS).catch(() => {});
        }
    } catch (e) {
        console.error(e);
    }
...
client.on('guildCreate', async (guild) => {
    try {
        ...
        console.error('Lỗi đăng ký slash commands cho guild mới:', e);
    }
});
    ...
    return interaction.reply({ content: `🔔 **Kêu gọi:** ${pings.join(', ')}`, embeds: [embed] });
        ...
        if (interaction.isModalSubmit()) {
            ...
            if (id === 'modal_buy_role') {
                const code = interaction.fields.getTextInputValue('buy_code_input').trim();
                const guildId = interaction.guildId;
                const serverShop = db.shop[guildId] || {};
                const item = serverShop[code];

                if (!item) {
                    return interaction.reply({ content: '❌ Giao dịch thất bại: Mã số không tồn tại trong cửa hàng của server này!', ephemeral: true });
                }
                ...
                if (!role) {
                    return interaction.reply({ content: '❌ Giao dịch thất bại: Role này không còn tồn tại trên server, vui lòng báo Admin!', ephemeral: true });
                }

                if (member?.roles?.cache?.has(role.id)) {
                    return interaction.reply({ content: '⚠️ Giao dịch thất bại: Bạn đã sở hữu Role này rồi!', ephemeral: true });
                }

                if (getBalance(userId, guildId) < price) {
                    return interaction.reply({ content: `❌ Giao dịch thất bại: Số dư trong tài khoản bạn không đủ để mua role này (**${price.toLocaleString()} VNĐ**).`, ephemeral: true });
                }
                ...
                try {
                    ...
                    const successEmbed = new EmbedBuilder()
                        ...
                        .setDescription(`Bạn đã mua thành công role <@&${role.id}> với giá **${price.toLocaleString()} VNĐ💸**.`)
                        .setTimestamp();

                    return interaction.reply({ embeds: [successEmbed], ephemeral: true });
                } catch (err) {
                    console.error('Lỗi khi cấp role:', err);
                    return interaction.reply({ content: '❌ Giao dịch thất bại: Bot không có đủ quyền ưu tiên để cấp role này. Vui lòng liên hệ Admin!', ephemeral: true });
                }
            }

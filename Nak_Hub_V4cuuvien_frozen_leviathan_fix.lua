...
local function StopFrozenTP()
	_G.FrozenTP = false
	if _G.SaveData then
		_G.SaveData["FrozenTP_Save"] = false
	end
	pcall(SaveSettings)
	pcall(function()
...
Event:AddToggle({
	...
	Callback = function(I)
		...
		task.spawn(function()
			...
			local function UseTyrantSkills()
				-- Dùng luôn logic skill kiểu Tyrant Of The Skies
				pcall(function()
					UseAllCategorySkills()
				end)
			end

			local function ForceToPart(part)
				local char = player.Character
				...
				if hrp then
					hrp.CFrame = part.CFrame * CFrame.new(0, 18, 0)
				end
			end
			...
			local function Attack(part)
				...
				repeat
					...
					ForceToPart(part)
					UseTyrantSkills()
				until not _G.Leviathan1 or not part.Parent
			end
			...
			while _G.Leviathan1 do
				...
				if SeaBeasts then
					...
					if Levi then
						-- Bám từng khúc thân để đánh
						for _, v in ipairs(Levi:GetChildren()) do
							if string.find(v.Name, "Segment") or string.find(v.Name, "Body") or string.find(v.Name, "Tail") then
								Attack(v)
								task.wait(0.05)
							end
						end
						...
						-- Nếu còn sống thì tiếp tục, chết rồi mới tự tắt
